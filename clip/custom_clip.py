...
class ClipWrapper(nn.Module):
    def __init__(self, 
        ...
        self.use_cupl_descriptions = use_cupl_descriptions
    ...
    def reset_classnames(self, classnames):
        ...
        if self.use_cupl_descriptions:
            with open("/mnt/workspace/cv_multimodal/jinmu/zero/CuPL/imagenet_prompts/CuPL_image_prompts.json") as f:
                gpt3_prompts = json.load(f)
            cupl_prompts = []
            for cls in classnames:
                cupl_prompts.extend(gpt3_prompts[cls])
            self.tokenized_descriptions = self.tokenize_callable(cupl_prompts).to(self.device)
        elif self.use_text_templates:
            hard_prompts = []
            for cls in classnames:
    ...
    def get_text_features(self):
        ...
        for i in range(0, len(prompts), 1000):
            batch_p = prompts[i:i + 1000]
            batch_tp = tokenized_prompts[i:i + 1000]
            all_features.append(self.text_encoder(batch_p, batch_tp))
        t_features = torch.cat(all_features, dim=0)
        
        norm_feats = t_features / t_features.norm(dim=-1, keepdim=True)

        if self.use_text_templates or self.use_cupl_descriptions:
...
def get_clip(clip_arch, pretrained, device, ctx_init, 
             ...
             cache_text_features=False, use_text_templates=False, use_cupl_descriptions=False,
    ...
    return ClipWrapper(
        ...
        use_cupl_descriptions=use_cupl_descriptions,
    )
