...
def confidence_filter(logits: torch.Tensor, probs: torch.Tensor, top:float, scale_w: torch.Tensor, return_idx: bool=False):
    
    # batch_entropy = shannon_entropy_minimization(probs)
    batch_entropy = tsallis_entropy_minimization(probs, q=scale_w)
    # full_idx = torch.argsort(logits.logsumexp(1), descending=False)
...
def tsallis_entropy_minimization(probs, q=2):
    if torch.is_tensor(q):
        N = len(q)
        entropy = torch.sum((torch.pow(probs,q) - 1/N)/(1 - q), dim=1)
    else:
        entropy = (1 - torch.sum(probs ** q, dim=1)) / (q - 1)
    return entropy

def shannon_entropy_minimization(probs):
...
class Zero(BaseTTAModule):
    ...
    @torch.no_grad()
    def zero(self, views, target, is_zero):
        ...
        logits_filt = confidence_filter((logits), prob, top=self.gamma, scale_w=self.scale_w) # retain most confident views
        
        p_bar = ((logits_filt - q)).softmax(1).sum(0) # marginalize
    
        return p_bar.unsqueeze(0)

    def forward(self, images, target, is_zero=False):
        return self.zero(images, target, is_zero)
